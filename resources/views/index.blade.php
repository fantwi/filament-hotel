<x-guest-layout>
    ...
    <main class="overflow-hidden bg-slate-50 text-slate-900 transition-colors dark:bg-[#161b48] dark:text-slate-100">
        <section class="relative isolate overflow-hidden bg-[#161b48] py-16 text-white sm:py-24 lg:py-32">
            ...
            <div class="mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
                <div>
                    ...
                    <h1 class="mt-6 max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl lg:text-6xl">Every stay should feel like it was made for you.</h1>
                    ...
                    <div class="mt-9 flex flex-col gap-3 sm:flex-row">
                        <a href="{{ route('rooms.index') }}" class="inline-flex min-h-12 items-center justify-center rounded-xl bg-amber-400 px-6 py-3 font-semibold text-slate-950 shadow-lg shadow-amber-950/20 transition hover:bg-amber-300">Find a room</a>
                        <a href="{{ route('restaurant') }}" class="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/30 bg-white/10 px-6 py-3 font-semibold text-white backdrop-blur transition hover:bg-white/20">Explore dining</a>
                    </div>
            ...
            <div class="mt-9 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                @forelse ($roomTypes->take(3) as $roomType)
                    <article class="group overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/70 transition hover:-translate-y-1 hover:shadow-xl dark:bg-slate-900 dark:ring-white/10">
                        ...
                        <div class="p-5"><h3 class="text-xl font-bold">{{ $roomType->name }}</h3><p class="mt-2 line-clamp-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ $roomType->description }}</p><a href="{{ url('/rooms/' . $roomType->id) }}" class="mt-5 inline-flex text-sm font-semibold text-indigo-700 dark:text-indigo-300">Explore this room →</a></div>
                    </article>
                @empty
        ...
        <section class="border-y border-slate-200 bg-white py-14 dark:border-white/10 dark:bg-slate-900/50 sm:py-20">
            <div class="mx-auto grid max-w-7xl gap-5 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
                <article class="relative overflow-hidden rounded-3xl bg-[#161b48] p-7 text-white shadow-xl shadow-indigo-950/20 sm:p-10"><div class="absolute -right-16 -top-16 h-56 w-56 rounded-full bg-cyan-300/15 blur-3xl"></div><div class="relative"><p class="text-sm font-bold uppercase tracking-[0.18em] text-amber-300">Dine with us</p><h2 class="mt-3 text-3xl font-bold">{{ $restaurant?->name ?? 'A meal worth lingering over' }}</h2><p class="mt-4 max-w-lg leading-7 text-indigo-100">{{ $restaurant?->description ?? 'Fresh flavours, thoughtful service, and a welcoming table for every occasion.' }}</p><div class="mt-7 flex flex-wrap gap-3"><a href="{{ route('restaurant.menu') }}" class="inline-flex min-h-11 items-center rounded-xl bg-amber-400 px-4 py-2 font-semibold text-slate-950 transition hover:bg-amber-300">View the menu</a><a href="{{ route('restaurant.reserve') }}" class="inline-flex min-h-11 items-center rounded-xl border border-white/25 px-4 py-2 font-semibold text-white transition hover:bg-white/10">Reserve a table</a></div></div></article>
                <article class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-cyan-100 to-indigo-100 p-7 text-slate-900 shadow-sm ring-1 ring-slate-200/70 dark:from-slate-800 dark:to-indigo-950 dark:text-white dark:ring-white/10 sm:p-10"><div class="absolute -bottom-16 -right-10 h-56 w-56 rounded-full bg-indigo-500/15 blur-3xl"></div><div class="relative"><p class="text-sm font-bold uppercase tracking-[0.18em] text-indigo-700 dark:text-indigo-300">Meet & celebrate</p><h2 class="mt-3 text-3xl font-bold">Spaces with room for ideas</h2><p class="mt-4 max-w-lg leading-7 text-slate-600 dark:text-slate-300">Professional, flexible venues for meetings, workshops, and celebrations of every size.</p><div class="mt-7 flex items-center gap-4"><a href="{{ url('/conference-rooms') }}" class="inline-flex min-h-11 items-center rounded-xl bg-indigo-600 px-4 py-2 font-semibold text-white transition hover:bg-indigo-700">Explore venues</a><span class="text-sm font-semibold text-slate-600 dark:text-slate-300">{{ $conferenceRooms->count() }} space(s) available</span></div></div></article>
            </div>
        </section>

        <section class="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8"><div class="rounded-3xl border border-indigo-200 bg-indigo-50 p-7 text-center dark:border-indigo-400/20 dark:bg-indigo-400/10 sm:p-10"><p class="text-sm font-bold uppercase tracking-[0.18em] text-indigo-600 dark:text-indigo-300">Plan with confidence</p><h2 class="mt-3 text-3xl font-bold">Your next visit starts here.</h2><p class="mx-auto mt-3 max-w-2xl text-slate-600 dark:text-slate-300">Choose your room, book your event space, or reserve a table in just a few steps.</p><div class="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><a href="{{ route('rooms.index') }}" class="inline-flex min-h-12 items-center justify-center rounded-xl bg-indigo-600 px-5 py-3 font-semibold text-white hover:bg-indigo-700">Start planning</a><a href="{{ route('contact') }}" class="inline-flex min-h-12 items-center justify-center rounded-xl border border-indigo-300 px-5 py-3 font-semibold text-indigo-800 hover:bg-white dark:border-indigo-300/40 dark:text-indigo-200 dark:hover:bg-white/10">Contact us</a></div></div></section>
    </main>
</x-guest-layout>
