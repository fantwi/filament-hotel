<x-guest-layout>
    ...
    <main class="min-h-screen bg-slate-50 py-7 text-slate-900 transition-colors dark:bg-[#161b48] dark:text-slate-100 sm:py-10">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:space-y-8 sm:px-6 lg:px-8">
            <section class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#161b48] via-indigo-900 to-blue-700 px-5 py-7 text-white shadow-2xl shadow-indigo-950/25 sm:px-8 sm:py-10">
                ...
                <div class="relative flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
                    <div class="flex items-center gap-4 sm:gap-5">
                        <div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-white/15 text-2xl font-bold ring-1 ring-white/25 backdrop-blur sm:h-20 sm:w-20 sm:text-3xl">
            ...
            <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <article class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-slate-900 dark:ring-white/10"><p class="text-sm font-medium text-slate-500 dark:text-slate-400">Total bookings</p><div class="mt-3 flex items-end justify-between"><p class="text-3xl font-bold tracking-tight">{{ $totalBookings }}</p><span class="rounded-xl bg-indigo-100 px-3 py-2 text-indigo-700 dark:bg-indigo-400/15 dark:text-indigo-200">All stays</span></div></article>
                <article class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-slate-900 dark:ring-white/10"><p class="text-sm font-medium text-slate-500 dark:text-slate-400">Confirmed</p><div class="mt-3 flex items-end justify-between"><p class="text-3xl font-bold tracking-tight">{{ $totalConfirmedBookings }}</p><span class="rounded-xl bg-emerald-100 px-3 py-2 text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-200">Ready to go</span></div></article>
                <article class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-slate-900 dark:ring-white/10"><p class="text-sm font-medium text-slate-500 dark:text-slate-400">Restaurant activity</p><div class="mt-3 flex items-end justify-between"><p class="text-3xl font-bold tracking-tight">{{ $totalRestaurantReservations + $totalRestaurantOrders }}</p><span class="rounded-xl bg-orange-100 px-3 py-2 text-orange-700 dark:bg-orange-400/15 dark:text-orange-200">Dining</span></div></article>
                <article class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-slate-900 dark:ring-white/10"><p class="text-sm font-medium text-slate-500 dark:text-slate-400">Total spent</p><div class="mt-3 flex items-end justify-between"><p class="text-2xl font-bold tracking-tight">GHS {{ number_format($totalSpent, 2) }}</p><span class="rounded-xl bg-cyan-100 px-3 py-2 text-cyan-700 dark:bg-cyan-400/15 dark:text-cyan-200">Paid</span></div></article>
            </section>
            ...
            <section x-data="{ tab: 'hotel' }" class="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200/70 dark:bg-slate-900 dark:ring-white/10">
                <div class="border-b border-slate-200 p-5 dark:border-white/10 sm:p-7">
                    ...
                    <div class="mt-6 flex gap-2 overflow-x-auto pb-1" role="tablist">
                        <button @click="tab = 'hotel'" :class="tab === 'hotel' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'" class="min-h-11 shrink-0 rounded-xl px-4 py-2 text-sm font-semibold transition">Hotel</button>
                        <button @click="tab = 'conference'" :class="tab === 'conference' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'" class="min-h-11 shrink-0 rounded-xl px-4 py-2 text-sm font-semibold transition">Conference</button>
                        <button @click="tab = 'restaurant'" :class="tab === 'restaurant' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'" class="min-h-11 shrink-0 rounded-xl px-4 py-2 text-sm font-semibold transition">Reservations</button>
                        <button @click="tab = 'orders'" :class="tab === 'orders' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'" class="min-h-11 shrink-0 rounded-xl px-4 py-2 text-sm font-semibold transition">Food orders</button>
                    </div>
                </div>
