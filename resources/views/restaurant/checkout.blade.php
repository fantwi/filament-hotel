<x-guest-layout>
    <div class="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        <form action="{{ route('restaurant.checkout.store') }}" method="POST" class="rounded-2xl bg-white p-5 shadow-xl shadow-slate-200/70 ring-1 ring-slate-900/5 sm:p-8">
            ...
            <div class="space-y-2">@foreach ($cartItems as $line)<div class="flex justify-between"><span>{{ $line['quantity'] }} × {{ $line['item']->name }}</span><span>GHS {{ number_format($line['line_total'], 2) }}</span></div>@endforeach</div>
            ...
            <div class="mt-4 border-t pt-4 text-xl font-bold">Estimated total: GHS {{ number_format($totals['total'], 2) }}</div>
            <label for="promotion_code" class="mt-6 block font-medium">Discount code</label>
            <input id="promotion_code" name="promotion_code" type="text" value="{{ old('promotion_code') }}" class="mt-2 min-h-12 w-full rounded-xl border-gray-300 px-4 text-base" placeholder="Optional discount code">
