<x-guest-layout>
    <section class="px-4 py-10 sm:px-6 sm:py-14"><div class="mx-auto max-w-xl">
        ...
        <div class="mt-6 rounded-2xl bg-white p-5 shadow-xl shadow-slate-200/70 ring-1 ring-slate-900/5 sm:p-8">
            ...
            @if ($reservation->subtotal !== null)<dl class="mt-6 space-y-2 border-t pt-4 text-sm"><div class="flex justify-between"><dt>Subtotal</dt><dd>GHS {{ number_format($reservation->subtotal, 2) }}</dd></div><div class="flex justify-between text-green-700"><dt>Discount{{ $reservation->promotion_code ? ' ('.$reservation->promotion_code.')' : '' }}</dt><dd>- GHS {{ number_format($reservation->discount, 2) }}</dd></div><div class="flex justify-between font-medium"><dt>Net after discount</dt><dd>GHS {{ number_format($reservation->subtotal - $reservation->discount, 2) }}</dd></div><div class="flex justify-between"><dt>Service charge</dt><dd>GHS {{ number_format($reservation->service_charge, 2) }}</dd></div><div class="flex justify-between"><dt>VAT</dt><dd>GHS {{ number_format($reservation->vat, 2) }}</dd></div><div class="flex justify-between"><dt>NHIL</dt><dd>GHS {{ number_format($reservation->nhil, 2) }}</dd></div></dl>@endif
